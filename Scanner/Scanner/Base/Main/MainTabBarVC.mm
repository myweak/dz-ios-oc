...
- (RrMineOrderMianListVC *)mineOrderVC{
    if (!_mineOrderVC) {
        _mineOrderVC = [RrMineOrderMianListVC new];
    }
    return _mineOrderVC;;
}
...
- (void)viewDidLoad
{
    ...
    LXNavigationController *navSp3 = [[LXNavigationController alloc] initWithRootViewController:self.mineOrderVC];
    self.mineOrderVC.title = @"订单";
    self.mineOrderVC.tabBarItem.image = [UIImage imageNamed:@"user-order-icon-n"];
    self.mineOrderVC.tabBarItem.selectedImage = [UIImage imageNamed:@"user-order-icon-y"];
    
    
    LXNavigationController *navSp4 = [[LXNavigationController alloc] initWithRootViewController: self.mineVC];
    ...
    self.mineOrderVC.hidenLeftTaBar = NO;
...
- (UIButton *)tabBarController:(JJTabBarController *)tabBarController tabBarButtonForTabBarChild:(UIViewController *)childViewController forIndex:(NSInteger)index {
    ...
    btn.imageEdgeInsets = UIEdgeInsetsMake(-20, 23, 0, 0);
    if (index == 0){
        btn.imageEdgeInsets = UIEdgeInsetsMake(-20, 23, 0, 0);
    }
    btn.tag = index;
    ...
    if (self.previousClickedTag == index) {
        ...
        if (timeInterval < 0.5) {
            ...
            if(index == 0){
                ...
                if ([self.mineOrderVC.navigationController.viewControllers count] >1) {
                    [self.mineOrderVC.navigationController popToRootViewControllerAnimated:YES];
                }
            }else if (index == 3){
