...
- (void)startGetAppVersionTiem{
    [self.time invalidate];
    self.time = nil;
    self.time = [NSTimer scheduledTimerWithTimeInterval:3 target:self selector:@selector(getAppVersionUrl) userInfo:nil repeats:YES];
...
/// 预览图片
+ (void)selectedImageView:(UIImageView *)imageView ImageArray:(NSArray *)photoImageArray Index:(NSInteger)index {
    
    NSMutableArray *datas = [NSMutableArray array];
    [photoImageArray enumerateObjectsUsingBlock:^(NSString *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
