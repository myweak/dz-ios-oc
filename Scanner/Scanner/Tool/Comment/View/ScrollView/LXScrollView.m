...
- (void)configBarItems{
    for (UIButton *btn in self.saveItemBtnArr) {
    ...
    if (self.type == LXScrollViewItemWidthType_equalMaxWidth) {
        ...
        for (NSInteger i = 0; i < _barTitles.count; i ++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(gsSortViewTitlesView:viewForIndex:)] ) {
                [self.dataSource gsSortViewTitlesView:item viewForIndex:i];
            }
