...
- (void)setImageUrl:(NSArray *)imageUrl{// 外部imageUrl
    ...
    NSMutableArray *arr = [NSMutableArray arrayWithArray:imageUrl];
    [imageUrl enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if(checkStrEmty(obj)){
...
- (void)updateAddPhotoView{
    ...
    for (int i = 0; i<self.photoImageArray.count; i++) {
        ...
        imageView.centerImageView.hidden = YES;
        ...
        if ([asset isKindOfClass:[NSString class]]) { // 字符类型
            if ([asset hasPrefix:@"http"]) {// url 字符
                ...
                if([asset hasSuffix:@".mp4"]){
                    imgStr = [NSString stringWithFormat:@"%@?vframe/jpg/offset/2",asset];
                    imageView.centerImageView.hidden = NO;
                }
        ...
        if([asset isKindOfClass:[UIImage class]]) { // 相册返回的 asset
            ...
            if (self.manger.currentAssets.count > assetIndex) {
                PHAsset *pHAsset = [self.manger.currentAssets objectAtIndex:assetIndex];
    ...
    if(self.photoImageArray.count == count_x){
        height_Y = 0;
    }
...
- (void)tapAddButtonAction:(UIButton *) btn{
    ...
    //    if (self.manger.imageAndOneVideo) {
    //        arr= @[@"从相册中选择图片",@"使用照相机",@"录制视频"];
    //    }
    [[[MZActionSheetView alloc]initWithActionSheetWithTitle:nil ListArray:arr completeSelctBlock:^(NSInteger selectIndex) {
        @strongify(self)
        if (selectIndex == 0) {
            // 手机相册
        ...
        if (selectIndex == 2) {
            [self.manger pickImageFromCameraVideo];
        }
    }] show] ;
}
...
- (void)selectedImageView:(UIImageView *)imageView Index:(NSInteger)index {
    ...
    [self.photoImageArray enumerateObjectsUsingBlock:^(NSString *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.manger.currentAssets.count >assetIndex) {
            PHAsset *assets =  self.manger.currentAssets[assetIndex];
            if (assets.mediaType == PHAssetMediaTypeVideo) {
                // 网络图片
                YBIBVideoData *data = [YBIBVideoData new];
                data.videoPHAsset = assets;
