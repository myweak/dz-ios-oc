...
-(void)setPostModel:(RrDidProductDeTailModel *)postModel{
    ...
    ///payType; 支付方式:0 全部 1在线支付，2线下支付:offLinePayBtn
    if ([self.postModel.productPayType integerValue] == 1) {
        self.offLinePayBtn.hidden = YES;
        self.onLinePayLBtn.hidden = NO;
    }else if ([self.postModel.productPayType integerValue] == 2){
        self.offLinePayBtn.hidden = NO;
        self.onLinePayLBtn.hidden = YES;
        self.offLinePayBtn_x.constant = 28;
    }else{
        self.payTitleLabel.text = @"选择支付方式";
    }
