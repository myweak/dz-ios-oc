...
@property (weak, nonatomic) IBOutlet UIImageView *requrieConnetImageView;
@property (weak, nonatomic) IBOutlet UIImageView *battertImageView;
@end
...
-(void)updateBattery{
    ...
    self.battertImageView.hidden = YES;
    if(percentage <=5){
        self.battertImageView.hidden = NO;
    }
...
//    self.requrieConnetImageView.hidden = YES;
    self.requrieConnetImageView.layer.zPosition = 101;
...
-(void)updateAppStatusMessage
{
    ...
    {
        BOOL hadConntScan =  [RrUserDefaults getBoolValueInUDWithKey:FirstScanKey];
        if(!hadConntScan){
            [RrUserDefaults saveBoolValueInUD:YES forKey:FirstScanKey];
            self.requrieConnetImageView.hidden = NO;
            [self.requrieConnetImageView handleTap:^(CGPoint loc, UIGestureRecognizer *tapGesture) {
                if((loc.x>300 || loc.x<500)&&
                   (loc.y>30 || loc.y<90)){
                    NSString *url = @"https://structure.io/buy-a-sensor";
                    [[UIApplication sharedApplication] openURL:url.url options:@{UIApplicationOpenURLOptionUniversalLinksOnly: @NO} completionHandler:nil];
