...
- (void)updateConfigUI{
    ...
    for (int i =0; i< self.itemLabelArr.count; i++) {
        ...
        if ([title containsString:@"订单编号："]) {
            self.copyBtn.frame = CGRectMake(self.contenViewBg.width-76, label.top-10, 76, label.height+20);
            self.copyBtn.titleLabel.font = KFont19;
...
- (UIButton *)copyBtn{
    if (!_copyBtn) {
        ...
        [_copyBtn setTitleColor:[UIColor c_btn_Bg_Color] forState:UIControlStateNormal];
        [self.contenViewBg addSubview:_copyBtn];
    }
    return  _copyBtn;
}
