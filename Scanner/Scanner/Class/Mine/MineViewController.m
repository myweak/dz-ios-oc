...
- (void)viewDidAppear:(BOOL)animated{
    ...
    [self getWaitReadCountWithresult];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([title isEqualToString:SOder_title]) {
        ...
        cell.bottonLineView.hidden = NO;
        if (!_badgeView) {
            _badgeView = [[JSBadgeView alloc] initWithParentView:cell.rightLabel alignment:JSBadgeViewAlignmentCenterLeft];
            _badgeView.badgeStrokeWidth = 12;
            _badgeView.badgePositionAdjustment = CGPointMake(-10, 0);
            _badgeView.badgeTextFont = KFont25;
        }
        _badgeView.hidden = checkStrEmty(self.messageNum);
        _badgeView.badgeText = self.messageNum;
...
///获取未读消息
- (void)getWaitReadCountWithresult{
    @weakify(self);
    [[RRNetWorkingManager sharedSessionManager] getWaitReadCountWithresult:ResultBlockMake(^(NSDictionary * _Nonnull dict, RrResponseModel * _Nonnull responseModel, NSError * _Nonnull error) {
        if(responseModel.data != nil){
            @strongify(self);
            NSString * num = [NSString stringWithFormat:@"%@",responseModel.data];
            self.messageNum = [num intValue]>99?@"99+":num;
            if ([num intValue] == 0) {
                self.messageNum = @"";
            }
